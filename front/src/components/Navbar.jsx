import { Link } from "react-router";

/**
 * Composant Navbar (Barre de navigation)
 * Affiche il logo MarsAI con stile gradient, menu di navigazione, bottone Soumettre e avatar utente
 * Recupera il firstName da localStorage per il saluto personalizzato
 * Barra sticky in alto in tutte le pagine con design Tailwind
 * @returns {JSX.Element} La barra di navigazione con design moderno
 */
export default function Navbar() {
  // Récupérer le prénom et le nom de l'utilisateur authentifié depuis localStorage
  const firstName = localStorage.getItem("firstName");
  const lastName = localStorage.getItem("lastName");

  /**
   * Fonction de déconnexion (Logout)
   * Nettoie tous les données de session du localStorage
   * et redirige vers la page d'accueil publique
   */
  function handleLogout() {
    // Supprimer toutes les données de session
    localStorage.removeItem("email");
    localStorage.removeItem("firstName");
    localStorage.removeItem("lastName");
    localStorage.removeItem("role");
    localStorage.removeItem("token");

    // Revenir à la page d'accueil publique
    window.location.href = "/";
  }

  return (
    <nav className="sticky top-0 z-50 mt-6 mx-2.5 bg-white/5 text-white text-xl uppercase rounded-full flex justify-between items-center p-4 border-white/10 border-2">
      {/* Logo MarsAI avec gradient */}
      <Link to="/">
        <div className="ml-3 flex gap-2.5 items-center">
          <div className="text-white text-3xl uppercase font-bold">Mars</div>
          <div className="text-3xl font-bold bg-gradient-to-b from-[#AD46FF] to-[#F6339A] bg-clip-text text-transparent">
            AI
          </div>
        </div>
      </Link>

      {/* Menu di navigazione */}
      <div className="flex gap-6 items-center font-bold">
        <Link to="/" className="hover:text-opacity-80 transition-opacity">
          Sélections
        </Link>
        <Link to="/" className="hover:text-opacity-80 transition-opacity">
          Programme
        </Link>
        <Link to="/" className="hover:text-opacity-80 transition-opacity">
          Jury
        </Link>
      </div>

      {/* Bottone Soumettre e Avatar/Login */}
      <div className="flex items-center gap-7">
        <button className="bg-gradient-to-br from-[#2B7FFF] to-[#9810FA] text-white px-8 py-3 rounded-full uppercase font-bold hover:from-blue-600 hover:to-purple-700 transition-colors duration-300">
          <Link to="/">Soumettre</Link>
        </button>

        <div className="mr-3">
          {firstName ? (
            <div className="flex items-center gap-3">
              <img
                src="data:image/png;base64,"
                alt="Avatar"
                className="w-16 rounded-full border-2 border-white/30"
              />
              <span className="text-sm">
                {firstName}
                {lastName && ` ${lastName}`}
              </span>
              <button
                onClick={handleLogout}
                className="text-sm text-red-400 hover:text-red-300 transition-colors"
              >
                Logout
              </button>
            </div>
          ) : (
            <Link to="/auth/login">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                strokeWidth="1.5"
                stroke="currentColor"
                className="w-8 h-8 hover:opacity-80 transition-opacity"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z"
                />
              </svg>
            </Link>
          )}
        </div>
      </div>
    </nav>
  );
}
