...
export default function VideosGrid({ 
  ...
  if (videos.length === 0) {
    return <div className="text-gray-400 text-center py-8">Aucune vidéo trouvée.</div>;
  }
        ...
        {videos.map((movie) => {
          ...
          return (
            <button
              ...
              key={movie.id_movie}
              onClick={() => onMovieSelect(movie)}
              className="text-left bg-gray-950 border border-gray-800 rounded-xl p-3 sm:p-4 hover:border-gray-600 transition w-full"
            >
              <div className="aspect-video bg-gray-800 rounded-lg overflow-hidden">
                {poster ? (
                  <img src={poster} alt={movie.title} className="w-full h-full object-cover" />
                ) : (
                  <div className="w-full h-full flex items-center justify-center text-gray-500 text-xs sm:text-sm p-2 text-center">
                    Aucune vignette
                  </div>
                )}
              </div>
              <div className="mt-3">
                <h3 className="text-base sm:text-lg font-semibold text-white truncate">{movie.title}</h3>
                <p className="text-xs sm:text-sm text-gray-400 mt-1 line-clamp-2">
                  {movie.synopsis || movie.description || "-"}
                </p>
                <div className="mt-2 text-[10px] sm:text-xs text-gray-400 flex flex-wrap gap-2 sm:gap-3">
                  <span>{movie.duration ? `${movie.duration}s` : "-"}</span>
                  <span>{movie.main_language || "-"}</span>
                  <span>{movie.nationality || "-"}</span>
                  <span className="px-1.5 py-0.5 bg-gray-800 rounded-full">{movie.selection_status || "submitted"}</span>
                </div>
              </div>
            </button>
          );
